class Game {
  ...
  displayRank(){
    ...
    camera.position.y = 0;
    clear()
    image(leaderboardImage, -800, -200, displayWidth, displayHeight)
    Player.getPlayerInfo()
    textAlign(CENTER)
    textSize(50)
    stroke("green")
    ...
    text("🏆🏆🏆 Leader Baord 🏆🏆🏆", 0, -100)

    if (allPlayers[plr].rank === 1){
      text("🚓1st🚕" + allPlayers[plr].name,0,50)
    }
    else if(allPlayers[plr].rank === 2){
      text("🚲2nd🛹" + allPlayers[plr].name,displayWidth/4 , displayHeight/8)

    }
    else if(allPlayers[plr].rank === 3){
      text("🛵3rd🛴" + allPlayers[plr].name,displayWidth/-4 , displayHeight/8)
    }
    else{
